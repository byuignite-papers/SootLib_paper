...
int main(int argc, char** argv) {
    ...
    ifstream ifile("burner_profs.inp");
    string line;
    ...
    Linear_interp LI_M(z_prof,     M_prof);
    ...
    for(int istep = 0; istep < nsteps; istep++, z+=dz) {
        ...
        for(int i=0; i<nsoot; i++)
            M[i] = Mhat[i]*LI_rho(z);
        S.setState(LI_T(z), P, LI_rho(z), LI_mu(z), LI_M(z), yGas, yPAH, M, nsoot);
        SM.setSourceTerms(S);
        ...
        S.setState(LI_T(zh), P, LI_rho(zh), LI_mu(zh), LI_M(zh), yGas, yPAH, M, nsoot);
        SM.setSourceTerms(S);
